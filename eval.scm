...
(define (scan-out-defines body)
  (define (split proc-body)
    (let iter ((proc-body proc-body) (defines '()) (excluding-defines '()))
      (cond ((null? proc-body) (cons (reverse defines) (reverse excluding-defines)))
            ((definition? (car proc-body)) (iter (cdr proc-body)
                                                 (cons (car proc-body) defines)
                                                 excluding-defines))
            (else (iter (cdr proc-body) defines (cons (car proc-body) excluding-defines))))))
  (let* ((splits (split body))
         (defines (car splits))
         (excluding-defines (cdr splits)))
    (if (null? defines)
        excluding-defines
